...
int main(int argc, char *argv[])
{
    ...
    u_int16_t r[2], g[2], b[2];
    u_int8_t pixel_r[2], pixel_g[2], pixel_b[2];
    int x, y;
    u_int32_t pixel_value[2];
    void *framebuffer;
    ...
    /* Ok, now we'll get ready to plot the pixel.
       ...
       We'll need to set two palette entrys.
       ...
       We'll need to set  palette entrys.
	...
	(fixed_info.visual == FB_VISUAL_DIRECTCOLOR)) {
		/* We want a white and a black pixel.
		...
		/* This will work in both pseudocolor and
		   directcolor modes. */
		pixel_value[WHITE] = 0xFFFFFFFF;
		pixel_value[BLACK] = 0xFFFFFFFF;
		...
		/* white pixel */
		pixel_value[WHITE] = (((pixel_r[WHITE] >> (8-var_info.red.length)) <<
				var_info.red.offset) +
				   ((pixel_g [WHITE]>> (8-var_info.green.length)) <<
				var_info.green.offset) +
				   ((pixel_b[WHITE] >> (8-var_info.blue.length)) <<
				var_info.blue.offset));
		/* black pixel */
		pixel_value[BLACK] = (((pixel_r[BLACK] >> (8-var_info.red.length)) <<
				var_info.red.offset) +
				   ((pixel_g [BLACK]>> (8-var_info.green.length)) <<
				var_info.green.offset) +
				   ((pixel_b[BLACK] >> (8-var_info.blue.length)) <<
				var_info.blue.offset));
    } else {
	...
	/* Fill the visible half of the screen buffer white
	   aand the invisible half of the screen buffer black. */
	for(x=0; x<var_info.xres * var_info.bits_per_pixel/8; x++) {
			...
			switch (var_info.bits_per_pixel) {
				case 8:
					...
					*((u_int8_t *)framebuffer +
						...
						(u_int8_t)pixel_value[WHITE];
					/* black pixels */
					*((u_int8_t *)framebuffer +
						fixed_info.line_length * (y + var_info.yres) + x) =
						(u_int8_t)pixel_value[BLACK];
				break;
				case 16:
					/* white pixels */
					*((u_int16_t *)framebuffer +
						fixed_info.line_length/2 * y + x) =
						(u_int16_t)pixel_value[WHITE];
					/* black pixels */
					*((u_int16_t *)framebuffer +
						fixed_info.line_length/2 * (y + var_info.yres) + x) =
						(u_int16_t)pixel_value[BLACK];
				break;
				case 24:
					...
					*((u_int8_t *)framebuffer +
						...
						(u_int8_t)pixel_r[WHITE];
					*((u_int8_t *)framebuffer +
						(fixed_info.line_length * y + 3 * x) + 1) =
						(u_int8_t)pixel_g[WHITE];
					*((u_int8_t *)framebuffer +
						(fixed_info.line_length * y + 3 * x) + 2) =
						(u_int8_t)pixel_b[WHITE];
					/* black pixels */
					*((u_int8_t *)framebuffer +
						(fixed_info.line_length * (y + var_info.yres) + 3 * x)) =
						(u_int8_t)pixel_r[BLACK];
					*((u_int8_t *)framebuffer +
						(fixed_info.line_length * (y + var_info.yres) + 3 * x) + 1) =
						(u_int8_t)pixel_g[BLACK];
					*((u_int8_t *)framebuffer +
						(fixed_info.line_length * (y + var_info.yres) + 3 * x) + 2) =
						(u_int8_t)pixel_b[BLACK];
				break;
				case 32:
	...
	while (1)
	{
		...
		if (ioctl(fbdev, FBIOPAN_DISPLAY, &var_info) == -1) {
