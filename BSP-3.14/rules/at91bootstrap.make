...
$(STATEDIR)/at91bootstrap.compile:
	@$(call targetinfo)
	@cd $(BUILDDIR)/../../patches/$(AT91BOOTSTRAP); \
	if [ -e $(BUILDDIR)/../../.svnrev ]; then \
       echo "#define VERSION \"$(AT91BOOTSTRAP)-svn`cat $(BUILDDIR)/../../.svnrev`\\n\\r\"" > $(AT91BOOTSTRAP_DIR)/include/version.h; \
	elif [ `svnversion 2>/dev/null | grep -v "Unversioned"` ]; then \
        echo "#define VERSION \"$(AT91BOOTSTRAP)-svn`svnversion`\\n\\r\"" > $(AT91BOOTSTRAP_DIR)/include/version.h; \
	else \
        echo "#define VERSION \"$(AT91BOOTSTRAP)\\n\\r\"" > $(AT91BOOTSTRAP_DIR)/include/version.h; \
 	fi	
