...
U_BOOT_BOOTLOGOS_DIR	:= $(PTXDIST_WORKSPACE)/local_src/common/u-boot-bootlogos
# ----------------------------------------------------------------------------
# Get
# ----------------------------------------------------------------------------
...
$(STATEDIR)/u-boot.prepare:
	@$(call targetinfo)
	@cd $(BUILDDIR)/../../patches/$(U_BOOT); \
	if [ -e $(BUILDDIR)/../../.svnrev ]; then \
       echo "-svn`cat $(BUILDDIR)/../../.svnrev`" > $(U_BOOT_DIR)/localversion-guf; \
	elif [ `svnversion 2>/dev/null | grep -v "Unversioned"` ]; then \
        echo "-svn`svnversion`" > $(U_BOOT_DIR)/localversion-guf; \
 	fi	
	@cd $(U_BOOT_DIR) && \
		$(U_BOOT_PATH) \
...
#endif
	#
	# Install the correct bootlogo.
	#
ifeq ($(PTXCONF_PLATFORM), ECO-G45)
	@install -m644 $(U_BOOT_BOOTLOGOS_DIR)/eco-g45/GF_Bootlogo_ECO-G45_16bpp.bmp \
		$(IMAGEDIR)/GF_Bootlogo_ECO-G45_16bpp.bmp
endif
	@$(call touch)
