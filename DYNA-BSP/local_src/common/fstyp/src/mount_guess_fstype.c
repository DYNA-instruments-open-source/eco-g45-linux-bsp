...
/* On FreeBSD reading(read(2)) the character devices(disks) fails if `nbytes' is
   less than 512. So I've padded all the superblock structures/unions to be
   multiple of 512 bytes. Correct me if anything more sane can be done.
   - Sachidananda */  
...
{
  return (!strncmp((const char *)rs->s_magic, REISERFS_SUPER_MAGIC_STRING, 
                    strlen ( REISERFS_SUPER_MAGIC_STRING)) ||
          !strncmp((const char *)rs->s_magic, REISER2FS_SUPER_MAGIC_STRING, 
...
fstype(const char *device) {
  ...
  /* Sachidananda: FreeBSD does not support block devices. So the above test
   * will not be applicable to FreeBSD. Since FreeBSD 4.0 the the distinction
   * between block and character device does not exist.
   */
  ...
  if (!type) {
    ...
    if (xiafsmagic(xsb.xiasb) == _XIAFS_SUPER_MAGIC)
      type = "xiafs";
    else if(!strncmp((const char *)xsb.romfs_magic, "-rom1fs-", 8))
      type = "romfs";
    else if(!strncmp((const char *)xsb.xfsb.s_magic, XFS_SUPER_MAGIC, 4))
      type = "xfs";
    else if(!strncmp((const char *)xsb.qnx4fs_magic+4, "QNX4FS", 6))
      type = "qnx4";
    else if(xsb.bfs_magic == 0x1badface)
      type = "bfs";
    else if(!strncmp((const char *)xsb.ntfssb.s_magic, NTFS_SUPER_MAGIC,
      ...
      type = "cramfs";
		...
		type = "EVA kernel";
                 ...
                 !strncmp((const char *)xsb.fatsb.s_fs, "FAT16   ", 8) ||
                 !strncmp((const char *)xsb.fatsb.s_fs2, "FAT32   ", 8)))
  ...
  if (!type) {
    ...
    if (lseek(fd, JFS_SUPER1_OFF, SEEK_SET) != JFS_SUPER1_OFF
      ...
      goto io_error;
    if (!strncmp((const char *)jfssb.s_magic, JFS_MAGIC, 4))
      type = "jfs";
  }

  /* ufs2 */
  if (!type) {
    if (lseek(fd, UFS2_DISK_OFFSET_IN_BYTES, SEEK_SET) != 65536
        || read(fd, (char *) &ufssb, sizeof(ufssb)) != sizeof(ufssb))
      goto io_error;

    if (ufssb.s_magic == FS_UFS2_MAGIC) 
      type = "ufs2";
  }

  /* ufs1 */
  if (!type) {
    if (lseek(fd, UFS_DISK_OFFSET_IN_BYTES, SEEK_SET) != 8192
        || read(fd, (char *) &ufssb, sizeof(ufssb)) != sizeof(ufssb))
      goto io_error;

    if (ufssb.s_magic == UFS_SUPER_MAGIC) /* also test swapped version? */
      type = "ufs";
  }

  /* UFS2:
   * As per to comment from ffs code, if the above two tests fail for UFS then
   * this is a real piggy system and superblock has to be checked at 256K from
   * front.  
   */
  if (!type) {
    if (lseek(fd, UFS2_PIGGY_OFFSET_IN_BYTES, SEEK_SET) != 262144
        || read(fd, (char *) &ufssb, sizeof(ufssb)) != sizeof(ufssb))
      goto io_error;
  ...
  if (!type) {
    ...
    if(strncmp((const char *)isosb.iso.id, ISO_STANDARD_ID, sizeof(isosb.iso.id)) == 0
