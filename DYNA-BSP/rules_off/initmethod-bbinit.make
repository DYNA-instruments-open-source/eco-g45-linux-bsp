...
INITMETHOD_BBINIT_SVN_VERSION := $(PTXCONF_PROJECT_VERSION)`([ -e $(BUILDDIR)/../../.svnrev ] && (echo -n "-svn" && cat $(BUILDDIR)/../../.svnrev)) || (svnversion >/dev/null 2>/dev/null && (echo -n "-svn" && svnversion $(BUILDDIR)/../../))`
...
ifdef PTXCONF_INITMETHOD_BBINIT_ETC_INITD_BANNER
	...
	@$(call install_replace, initmethod-bbinit, /etc/init.d/banner, \
		@PRJVERSION@, $(INITMETHOD_BBINIT_SVN_VERSION))
